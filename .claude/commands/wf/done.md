...
```bash
$(git rev-parse --show-toplevel)/.orchay/bin/transition {Task-ID} done -p {project} --start
```
...
$(git rev-parse --show-toplevel)/.orchay/bin/transition {Task-ID} done -p {project}
```
