...
| `/wf:fix ACT-01-01` | ACT 내 모든 `[ap]` Task 병렬 |
...
**에러 출력:**
```
[ERROR] 현재 상태 [{currentStatus}]에서 'fix' 명령어를 사용할 수 없습니다.
필요한 상태: [ap]
```
...
**실패:**
```
ORCHAY_DONE:{project}/{task-id}:fix:error:{에러 요약}
```
