...
```bash
$(git rev-parse --show-toplevel)/.orchay/bin/transition {Task-ID} verify -p {project} --start
```
...
$(git rev-parse --show-toplevel)/.orchay/bin/transition {Task-ID} verify -p {project}
```
