...
| BR-002 | 4.1.1 | 8.1, 8.2 | menu.service.ts | UT-007 | E2E-006 | 4단계 메뉴 생성 시 400 에러 확인 | 설계완료 |
| BR-003 | 4.1.1 | 8.1 | menu.service.ts | UT-009 | - | 비활성 메뉴 응답 제외 확인 | 설계완료 |
| BR-004 | 4.1.1 | 8.1 | menu.service.ts | UT-003 | E2E-003 | sortOrder 오름차순 정렬 확인 | 설계완료 |
| BR-005 | (설계도출) | 8.2 | menu.service.ts | UT-010 | E2E-008 | 순환 참조 시 400 에러 확인 | 설계완료 |
| BR-006 | (설계도출) | 8.2 | menu.service.ts | UT-008 | E2E-007 | 자식 있는 메뉴 삭제 시 400 에러 확인 | 설계완료 |

> **참고**: BR-005, BR-006은 PRD에 명시되지 않았으나 데이터 무결성 및 시스템 안정성 확보를 위해 설계 단계에서 도출된 규칙입니다.
...
| 구분 | 내용 |
|------|------|
| **PRD 원문** | 메뉴는 sortOrder 기준 오름차순 정렬 |
| **설계 표현** | findAll()에서 sortOrder ASC 조건 |
| **구현 위치** | menu.service.ts - findAll() |
| **검증 방법** | 응답 메뉴가 sortOrder 순 정렬됨 확인 |
| **관련 테스트** | UT-003, E2E-003 |

#### BR-005: 순환 참조 금지 (설계 도출)

| 구분 | 내용 |
|------|------|
| **도출 근거** | 자기참조 관계에서 순환 구조 발생 시 무한 루프로 서버 크래시 위험 |
| **설계 표현** | parentId 변경 시 순환 참조 검증 로직 |
| **구현 위치** | menu.service.ts - validateNoCircularReference() |
| **검증 방법** | 자기 자신 또는 자식을 부모로 지정 시 BadRequestException 발생 확인 |
| **관련 테스트** | UT-010, E2E-008 |

#### BR-006: 자식 메뉴 삭제 보호 (설계 도출)

| 구분 | 내용 |
|------|------|
| **도출 근거** | 하위 메뉴가 있는 상태에서 상위 메뉴 삭제 시 고아 레코드 발생 |
| **설계 표현** | 삭제 전 자식 메뉴 존재 확인 |
| **구현 위치** | menu.service.ts - validateNoChildren() |
| **검증 방법** | 자식 있는 메뉴 삭제 시도 시 BadRequestException 발생 확인 |
| **관련 테스트** | UT-008, E2E-007 |
...
| 설계 인터페이스 | API명 | Method | Endpoint | 요구사항 | 범위 |
|----------------|-------|--------|----------|----------|------|
| 메뉴 목록 조회 | getMenus | GET | /api/menus | FR-001, FR-002 | **TSK-03-01 (본 Task)** |
| 메뉴 생성 | createMenu | POST | /api/menus | FR-001 | Phase 2 (관리자 기능) |
| 메뉴 수정 | updateMenu | PUT | /api/menus/:id | FR-003, FR-004, FR-005 | Phase 2 (관리자 기능) |
| 메뉴 삭제 | deleteMenu | DELETE | /api/menus/:id | - | Phase 2 (관리자 기능) |

> **참고**: 본 Task(TSK-03-01)는 GET /api/menus 구현에 집중하며, CRUD API(POST, PUT, DELETE)는 Phase 2 관리자 기능에서 구현됩니다.
