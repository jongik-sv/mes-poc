...
| ID | 제목 | 판단 | 처리 내용 |
|----|------|------|----------|
| SEC-02 | 서버 사이드 경로 접근 제어 미정의 | ✅ 적용 | UC-02에 Next.js 미들웨어 및 API 레벨 권한 검증 설계 추가 |
| ARCH-01 | TRD RoleMenu 스키마 불일치 | 📝 조정 적용 | 설계 문서에 TRD 동기화 필요 사항 명시 (TRD 수정은 별도 처리) |
| ARCH-02 | BR-02 계층형 메뉴 필터링 규칙 모순 | ✅ 적용 | BR-02 규칙 재정의: "자식 권한 있으면 부모도 자동 표시" |
| SEC-01 | ADMIN 역할 하드코딩 및 우회 가능성 | ✅ 적용 | BR-03에서 ID 기반 검증으로 변경, 문자열 비교 금지 명시 |
| QUAL-03 | BR-02 규칙과 시드 데이터 충돌 | ✅ 적용 | 권한 매트릭스 및 시드 데이터 일관성 확보 |

### P2 이슈 처리 결과 (80% 이상 처리)

| ID | 제목 | 판단 | 사유 |
|----|------|------|------|
| ARCH-03 | ADMIN 역할 처리 방식 이중 구현 | ✅ 적용 | BR-03에서 ID 기반으로 단일화 |
| ARCH-04 | 시드 데이터 와일드카드 문법 미구현 | 📝 조정 적용 | 설계상 표현임을 주석으로 명시 |
| ARCH-05 | N+1 쿼리 가능성 | ⏸️ 보류 | 구현 단계에서 최적화 (설계 범위 외) |
| ARCH-06 | 인덱스 전략 부재 | ⏸️ 보류 | 구현/성능 튜닝 단계에서 처리 |
| ARCH-07 | 계층형 메뉴 필터링 로직 누락 | ✅ 적용 | BR-02에 buildMenuTreeWithInheritance 로직 추가 |
| SEC-03 | 세션/인증 정보 검증 불충분 | ✅ 적용 | API 로직에 isActive 상태 검증 추가 |
| SEC-04 | 권한 검증의 단일 계층 의존 | ✅ 적용 | 미들웨어 + API 레벨 이중 방어층 설계 |
| SEC-05 | Cascade 삭제로 인한 권한 손실 위험 | ⏸️ 보류 | Cascade 삭제는 BR-05의 의도된 설계 |
| SEC-07 | 기본 거부 원칙 미명시 | ✅ 적용 | 미들웨어에서 기본 거부 원칙 적용 |
| QUAL-02 | 메뉴 권한 매트릭스와 시드 데이터 불일치 | ✅ 적용 | OPERATOR 시드 데이터에서 부모 메뉴 제거, 비고 열 추가 |
| QUAL-04 | data-testid 정의 누락 | ⏸️ 보류 | 테스트 명세서(026)에서 정의 예정 |
| QUAL-05 | ADMIN 역할 특별 처리 방식 미정의 | ✅ 적용 | BR-03에서 상세 정의 |
| QUAL-07 | UC-02 권한 체크 상세 로직 미정의 | ✅ 적용 | UC-02에 6단계 상세 흐름 및 코드 예시 추가 |

### 보류 사유

| 이슈 | 보류 사유 |
|------|----------|
| ARCH-05, ARCH-06 | 성능 최적화는 구현 단계에서 프로파일링 후 처리 |
| SEC-05 | Cascade 삭제는 BR-05에서 명시한 의도된 설계 |
| SEC-06, SEC-08 | 입력 검증/감사 로그는 별도 Task 범위 |
| QUAL-04 | 테스트 명세서(026)에서 data-testid 정의 예정 |

### 수정된 문서

- `010-design.md` (v1.0 → v1.1)
  - UC-02: 서버 사이드 접근 제어 설계 추가
  - BR-02: 규칙 재정의 및 구현 로직 추가
  - BR-03: ADMIN 검증 강화 (ID 기반)
  - 4.1: TRD 동기화 필요 사항 명시
  - 4.3: 권한 매트릭스 비고 열 추가
  - 4.4, 7.3: 시드 데이터 OPERATOR 부모 메뉴 제거
  - 6.1: API 로직에 isActive 검증 및 ADMIN ID 기반 검증 적용
