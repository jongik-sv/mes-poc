...
|-------------|----------|---------------|-------------|------------|-----------|------|
| FR-001 | 4.1.1 동적 메뉴 - 사용자 권한별 메뉴 필터링 | 010-design.md 섹션 6.1 | UT-001, UT-002 | E2E-001 | TC-001 | 설계완료 |
| FR-002 | 4.1.1 동적 메뉴 - 계층형 메뉴 지원 | 010-design.md 섹션 5.2, 6.1 | UT-003 | E2E-002 | TC-002 | 설계완료 |
...
| 설계 | 010-design.md | 6.1 | 메뉴 조회 API 필터링 로직 |
...
| 설계 인터페이스 | API 엔드포인트 | Method | Path | 요구사항 | 구현 Task |
|----------------|---------------|--------|------|----------|----------|
| 메뉴 조회 (권한 필터링) | 메뉴 목록 API | GET | /api/menus | FR-001, FR-002, FR-003 | TSK-03-03 |
| 역할-메뉴 매핑 조회 | 역할별 메뉴 조회 | GET | /api/roles/{roleId}/menus | - (관리 기능) | MVP 이후 |
| 역할-메뉴 매핑 생성 | 매핑 생성 | POST | /api/role-menus | - (관리 기능) | MVP 이후 |
| 역할-메뉴 매핑 삭제 | 매핑 삭제 | DELETE | /api/role-menus/{id} | - (관리 기능) | MVP 이후 |
...
| 1.1 | 2026-01-20 | Claude | 설계 리뷰 반영 - FR-001/FR-002 섹션 참조 수정, 인터페이스 추적 구현 Task 컬럼 추가 |
