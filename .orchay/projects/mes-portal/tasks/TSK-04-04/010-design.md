...
| 요소 | ARIA 속성 | 값 | 설명 |
|------|----------|-----|------|
| 로그인 폼 | role | "form" | 폼 영역 명시 |
| 로그인 폼 | aria-label | "로그인 폼" | 폼 용도 설명 |
| 이메일 필드 | aria-required | "true" | 필수 입력 명시 |
| 이메일 필드 | aria-invalid | "true/false" | 유효성 상태 |
| 이메일 필드 | aria-describedby | "{id}-error" | 에러 메시지 연결 |
| 비밀번호 필드 | aria-required | "true" | 필수 입력 명시 |
| 비밀번호 필드 | aria-invalid | "true/false" | 유효성 상태 |
| 에러 Alert | role | "alert" | 즉시 안내 필요 |
| 에러 Alert | aria-live | "polite" | 스크린 리더 알림 |
| 로딩 상태 | aria-busy | "true" | 처리 중 상태 |

**포커스 관리:**
- 폼 에러 발생 시: 첫 번째 에러 필드로 포커스 이동
- 인증 실패 시: Alert 영역으로 포커스 이동
- 로딩 완료 시: 다음 액션 가능 영역으로 포커스
...
| 데이터 필드 | 규칙 | 위반 시 메시지 |
|------------|------|---------------|
| 이메일 | 필수, 이메일 형식, 최대 255자 | "이메일을 입력해주세요" / "올바른 이메일 형식이 아닙니다" |
| 비밀번호 | 필수, 최소 8자, 복잡도 요구 | "비밀번호를 입력해주세요" / "비밀번호는 최소 8자 이상이어야 합니다" |

**비밀번호 복잡도 규칙 (클라이언트 유효성):**
- 최소 8자 이상
- 영문 대문자, 소문자, 숫자, 특수문자 중 3가지 이상 조합 권장 (안내 메시지)

> **참고**: 서버 측 비밀번호 정책은 TSK-04-03(Auth.js 설정)에서 정의
...
| BR-05 | 로그인 시도 횟수 제한 (Rate Limiting) | 로그인 시도 시 | 없음 |
...
**BR-05: 로그인 시도 횟수 제한 (Rate Limiting)**

설명: Brute Force 공격 방어를 위한 로그인 시도 제한 정책

| 단계 | 조건 | 동작 |
|------|------|------|
| 1단계 | 3회 연속 실패 | 경고 메시지 표시 |
| 2단계 | 5회 연속 실패 | 5분 대기 시간 적용 |
| 3단계 | 10회 연속 실패 | 계정 일시 잠금 (30분) |

**에러 메시지:**
- 대기 시간 적용 시: "로그인 시도 횟수를 초과했습니다. {n}분 후에 다시 시도해주세요."
- 계정 잠금 시: "보안을 위해 계정이 일시적으로 잠겼습니다. 30분 후에 다시 시도해주세요."

> **구현 위치**: Rate Limiting 로직은 API 레벨에서 처리하며, 프론트엔드는 에러 메시지 표시 담당
...
| 네트워크 오류 | 서버 연결 실패 | "네트워크 오류가 발생했습니다. 다시 시도해주세요" | 재시도 버튼 클릭 |
| 서버 오류 | 500 에러 | "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요" | 대기 후 재시도 |
| 시도 횟수 초과 | Rate Limiting | "로그인 시도 횟수를 초과했습니다. {n}분 후에 다시 시도해주세요" | 대기 |
| 계정 잠금 | 10회 이상 실패 | "보안을 위해 계정이 일시적으로 잠겼습니다" | 30분 대기 또는 관리자 문의 |
...
| 네트워크/서버 오류 | 폼 상단 | Ant Design Alert (type="error") + 재시도 버튼 |
| Rate Limiting | 폼 상단 | Ant Design Alert (type="warning") + 남은 시간 표시 |

**네트워크 오류 시 재시도 버튼:**
- Alert 내부 또는 하단에 "다시 시도" 버튼 배치
- 버튼 클릭 시 마지막 로그인 시도 재실행

### 9.3 보안 관련 에러 처리 원칙

**정보 노출 방지:**
- 서버 에러 발생 시 스택 트레이스 또는 상세 에러 정보를 사용자에게 노출하지 않음
- 모든 서버 오류는 일반화된 메시지로 표시: "서버 오류가 발생했습니다"
- 개발 환경에서만 상세 에러 로깅, 프로덕션에서는 최소 정보만 전달

**클라이언트/서버 이중 검증:**
- 클라이언트 측 유효성 검사는 사용자 경험 향상 목적
- 서버 측에서 동일한 유효성 검사 필수 적용 (보안 목적)
- 클라이언트 검증 우회 시도에 대비한 서버 검증 필수
...
### 11.2.1 TSK-04-03 인터페이스 정의 (Mock용)

TSK-04-03 완료 전 개발을 위한 signIn 함수 인터페이스:
...
export interface SignInResult {
  ok: boolean;
  error?: 'CredentialsSignin' | 'AccountInactive' | 'RateLimited' | 'AccountLocked';
  url?: string;
}
...
export async function mockSignIn(
  ...
  credentials: SignInCredentials & { redirect: false }
): Promise<SignInResult> {
  // TSK-04-03 완료 후 Auth.js signIn으로 대체
}
```
...
export const AUTH_MESSAGES = {
  ...
  ACCOUNT_LOCKED: '보안을 위해 계정이 일시적으로 잠겼습니다. 30분 후에 다시 시도해주세요',
} as const;
```
...
| 1.1 | 2026-01-20 | Claude | 설계 리뷰 반영 (P1 이슈 적용) |
