...
| TC-011 | 마법사 취소 | 데이터 입력 상태 | 취소 버튼 클릭 | 확인 다이얼로그 표시 | High | FR-008 |
...
**테스트 목적**: 마법사 진행 중 취소 시 확인 다이얼로그가 표시되는지 확인

**테스트 단계**:
1. 1단계에서 회사명 등 데이터 입력
2. [취소] 버튼 클릭
3. 확인 다이얼로그 표시 확인
4. [취소] 버튼 클릭으로 다이얼로그 닫기
5. 다시 [취소] 버튼 클릭
6. [확인] 버튼 클릭으로 마법사 종료

**예상 결과**:
- "진행 중인 내용이 저장되지 않습니다. 취소하시겠습니까?" 메시지 표시
- [취소] 클릭 시 다이얼로그 닫히고 마법사 유지
- [확인] 클릭 시 마법사 종료
...
> **참고**: Mock 데이터 전용 샘플로 별도의 테스트 계정이 필요하지 않습니다.
> 인증/인가 없이 페이지 접근이 가능합니다.
