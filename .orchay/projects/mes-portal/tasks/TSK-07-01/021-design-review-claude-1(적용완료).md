...
| 이슈 ID | 제목 | 판단 | 사유 |
|---------|------|------|------|
| ARCH-004 | 데이터 계층 추상화 부족 | 📝 조정 적용 | 010-design.md에 DashboardDataService 인터페이스 및 Mock/API 전환 전략 추가 |
| ARCH-010 | 데이터 연동 전환 시 변경 범위 불명확 | 📝 조정 적용 | 데이터 로딩 책임 위치 및 변경 영향 범위 문서화 |
| SEC-003 | 대시보드 접근 권한 검증 | ⏸️ 보류 | 인증/인가는 TSK-01-04에서 처리, 현재 Task 범위 초과 |
| QR-001 | 추적성 매트릭스 설계 섹션 참조 불일치 | ✅ 적용 | 025-traceability-matrix.md 섹션 번호 수정 (3.x → 5.x, 11.x) |
| QR-003 | data-testid 명명 불일치 | ✅ 적용 | 026-test-specification.md에서 `activity-section` → `recent-activity-section` 통일 |
| QR-006 | 테스트 코드 예시의 selector 불일치 | ✅ 적용 | 테스트 코드에서 `kpi-slot-*` → `kpi-card-*` 형식으로 수정 |
| QR-007 | 반응형 단위 테스트의 실효성 문제 | ✅ 적용 | 테스트 전략 변경: 단위테스트는 props 전달 확인, 실제 레이아웃은 E2E 의존 |
| QR-008 | 추적성 매트릭스의 breakpoint 규칙 불일치 | ✅ 적용 | 010-design.md 기준으로 breakpoint 규칙 수정 |

### P3 이슈 처리 결과 (선별 적용 3건)

| 이슈 ID | 제목 | 판단 | 사유 |
|---------|------|------|------|
| ARCH-016 | 에러 경계 전략 미정의 | 📝 조정 적용 | 010-design.md 9.3에 WidgetErrorBoundary 전략 추가 |
| QR-002 | 테스트 명세서 파일 경로 불일치 | ✅ 적용 | `screens/dashboard/` → `components/dashboard/` 수정 |
| QR-004 | BR-002 E2E 테스트 누락 | ✅ 적용 | E2E-009 테스트 추가 (위젯 최소 높이 검증) |

### P3~P4 보류 이슈 (12건)

| 이슈 ID | 제목 | 보류 사유 |
|---------|------|----------|
| ARCH-003 | 위젯 타입별 렌더링 전략 미정의 | Phase 1 범위 초과, 향후 확장 시 검토 |
| ARCH-008 | 위젯 배치 하드코딩 우려 | Phase 1 설계 유지, 향후 로드맵에서 검토 |
| QR-009 | WidgetCard 에러 상태 재시도 동작 미정의 | Phase 1에서는 기본 에러 표시만 구현 |
| ARCH-005 | Section/WidgetCard 역할 중복 가능성 | P4 - 구현 시 자연스럽게 해결 |
| ARCH-012 | CSS 변수 활용 계획 미명시 | P4 - TRD 가이드라인 준수로 충분 |
| ARCH-014 | Client Component 과다 사용 검토 | P4 - Ant Design 사용 필수, 최적화는 향후 |
| ARCH-015 | 타입 정의 위치 명확화 | P4 - 구현 단계에서 결정 |
| QR-005 | PRD 파이 차트 요구사항 미반영 | P4 - 별도 Task에서 처리 |
| QR-010 | 테스트 명세서 뷰포트 크기 불일치 | P4 - 경미한 불일치 |
| SEC-001 | XSS 취약점 (향후 API 연동 시) | P4 - React 기본 보호 적용 |
| SEC-004 | 위젯별 데이터 접근 권한 | P3 - 인증 Task에서 처리 |
| SEC-006 | 클라이언트 사이드 데이터 노출 | P4 - Mock 데이터에 민감 정보 없음 |

### 수정된 문서

| 문서 | 수정 내용 |
|------|----------|
| 010-design.md | 7.5절 데이터 서비스 추상화, 9.3절 에러 경계 전략 추가 |
| 025-traceability-matrix.md | 설계 섹션 참조 번호 수정, breakpoint 규칙 수정, 컴포넌트명 수정 |
| 026-test-specification.md | data-testid 통일, 테스트 파일 경로 수정, 반응형 테스트 전략 변경, E2E-009 추가 |
