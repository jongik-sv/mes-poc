...
| 이슈 ID | 우선순위 | 판단 | 사유 |
|---------|---------|------|------|
| CR-001 | P1 | ✅ 적용 | BR 정의를 긍정/부정 KPI 유형으로 수정 |
| CR-002 | P1 | ✅ 적용 | BR-005~BR-009 추가 |
| HI-001 | P2 | ✅ 적용 | KPI ID를 설계 문서 기준으로 통일 |
| HI-002 | P2 | ✅ 적용 | 작업지시(orders) 카드 테스트 추가 |
| HI-003 | P2 | ⏸️ 보류 | Phase 2 API 연동 시 적용 예정 |
| MD-001 | P3 | ✅ 적용 | data-testid `kpi-{element}-{id}` 형식 통일 |
| MD-002 | P3 | ✅ 적용 | Props 인터페이스 통일 (KPICardData, KPICardProps) |
| LO-001 | P4 | ✅ 적용 | Mock 데이터 구조 kpiCards 배열로 통일 |
| LO-002 | P4 | ✅ 적용 | 테스트 ID 매핑 수정 |
| IN-001 | P5 | ⏸️ 보류 | 구현 단계에서 고려 |
| IN-002 | P5 | ⏸️ 보류 | 구현 단계에서 고려 |

### 8.2 수정된 문서

| 문서 | 주요 변경 내용 |
|------|---------------|
| 025-traceability-matrix.md | BR-001~BR-009 정의 수정, 긍정/부정 KPI 개념 추가, 작업지시 카드 추가, data-testid 통일, 인터페이스 통일 |
| 026-test-specification.md | 테스트 케이스 추가 (UT-004-2, UT-005-2, UT-010-2, UT-010-3), E2E 4종 카드 테스트, 요구사항 추적 매트릭스 수정 |

### 8.3 보류 사유

- **HI-003** (useKPICards 추상화): TRD 섹션 2.3의 USE_MOCK 패턴은 Phase 2 실제 API 연동 시 적용 예정. 현재 Mock 데이터 단계에서는 직접 import로 충분.
- **IN-001, IN-002** (타입 안정성, 테스트 용이성): 구현 단계에서 코드 작성 시 자연스럽게 반영될 개선 사항.
...
| 1.1 | 2026-01-21 | Claude | 적용 결과 추가 (wf:apply) |
