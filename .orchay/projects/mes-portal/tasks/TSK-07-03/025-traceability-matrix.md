...
> QA-005 대응: PRD 참조 명확화 - 툴팁은 4.1.1 차트 위젯의 "데이터 호버 시 상세 정보 표시" 요구사항

| 설계 단계 | 문서 | 섹션 | 구현 항목 |
|----------|------|------|----------|
| PRD | prd.md | 4.1.1 차트 위젯 | "데이터 포인트 호버 시 상세 정보 툴팁" |
...
> PRD 요구사항 중 현재 Task 범위에서 제외된 항목과 그 근거

| 요구사항 | PRD 섹션 | 제외 사유 | 대체/후속 계획 |
|----------|----------|----------|---------------|
| 실시간 차트 갱신 | 4.1.1 | Phase 2 구현 예정 - 현재 MVP 단계에서는 mock 데이터 기반 정적 차트로 충분 | Phase 2에서 WebSocket 또는 SSE 기반 실시간 데이터 스트리밍 구현 |
| 차트 내보내기 | 4.1.1 | 핵심 기능 우선 - 내보내기는 부가 기능으로 후순위 | 향후 @ant-design/charts export 기능 또는 html2canvas 활용 검토 |
| 드릴다운 기능 | - | 현재 대시보드 요약 수준에서 불필요 - 상세 분석 페이지에서 구현 | 상세 분석 페이지 Task에서 구현 |
| 게이지 차트 | 4.1.1 | 별도 Task(TSK-07-04)로 분리 - 차트 유형별 Task 분할 정책 | TSK-07-04에서 구현 |
