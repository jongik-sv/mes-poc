...
| 명령어       | 상태 전환       | 설명                                                            |
| ------------ | --------------- | --------------------------------------------------------------- |
| `/wf:start`  | `[ ]` → `[dd]`  | 설계 문서 생성 (010-design.md, 025-traceability, 026-test-spec) |
| `/wf:ui`     | -               | UI 화면 설계 (011-ui-design.md, ui-assets/)                     |
| `/wf:review` | -               | 설계 리뷰                                                       |
| `/wf:apply`  | `[dd]` → `[ap]` | 설계 리뷰 반영                                                  |
| `/wf:build`  | `[ap]` → `[im]` | TDD 기반 구현 (backend/frontend 병렬)                           |
| `/wf:test`   | -               | 테스트 실행                                                     |
| `/wf:audit`  | -               | 코드 리뷰                                                       |
| `/wf:patch`  | -               | 코드 리뷰 반영                                                  |
| `/wf:fix`    | -               | 결함 수정                                                       |
| `/wf:verify` | `[im]` → `[vf]` | 통합 테스트                                                     |
| `/wf:done`   | `[vf]` → `[xx]` | 작업 완료, 매뉴얼 생성                                          |

### 사용법

```bash
/wf:start TSK-01-01          # 단일 Task
/wf:build WP-01              # Work Package 단위 (하위 Task 병렬 처리)
/wf:done mes-portal/TSK-01-01  # 프로젝트 지정
```

### Task 카테고리별 처리

- **development**: 설계 → 구현 → 테스트 전체 플로우
- **infrastructure**: 기술 설계 (010-tech-design.md)
- **defect**: 결함 분석 (010-defect-analysis.md)
