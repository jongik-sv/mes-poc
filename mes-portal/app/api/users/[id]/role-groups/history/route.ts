...
/**
 * GET /api/users/:id/role-groups/history
 * 사용자의 역할 그룹 할당 이력 조회 (?from=&to= 날짜 범위 필터)
 */
export async function GET(
  ...
  try {
    ...
    if (from) {
      where.validFrom = { gte: new Date(from) }
    }
    if (to) {
      where.validTo = { lte: new Date(to) }
    }

    const history = await prisma.userRoleGroupHistory.findMany({
      where,
